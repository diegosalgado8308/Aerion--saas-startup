...
- **`DATABASE_URL` is split per environment via dedicated Neon branches** — Neon project `dark-feather-30915454` (`Aerion Software`) has three branches, each with its own Vercel-scoped `DATABASE_URL`: `production` (`br-little-lake-au6dzrei`, the original branch) → Vercel Production, `preview` (`br-royal-block-au59sfid`) → Vercel Preview, `development` (`br-dark-sky-auvpu9me`) → Vercel Development *and* local `.env`. `preview`/`development` were branched off `production` on 2026-08-15, so they start as a full copy (schema + data, including the `_prisma_migrations` history) and diverge independently from there — preview deploys and local dev can now write freely without touching real production data. Create new branches (`neonctl branches create --parent production`) if these ever need resetting back to a clean copy of prod.
