...
- **Vercel project** — linked (`vercel link`), creating `.vercel/project.json` for the `aerion-software` project under the account's default team. `vercel.json`'s `buildCommand` runs `prisma migrate deploy && next build`, so pending migrations apply automatically before each production build rather than needing a separate manual step. `DATABASE_URL` is now set on Vercel (Production/Preview as Sensitive, Development as regular — `vercel env add`), pointing at the same Neon database used for local dev; no dedicated branch per environment yet. Still no deployment triggered — that's a separate, explicit step (`vercel` for a preview, `vercel --prod` for production) once the remaining env vars are set (`AUTH_SECRET`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `APP_URL`, `CRON_SECRET`, `BLOB_READ_WRITE_TOKEN` — see **Configuration & environment**). `AUTH_TRUST_HOST` doesn't need setting on Vercel; Vercel deployments set the equivalent automatically. **`DATABASE_URL` specifically must be the Postgres connection string** (`postgresql://user:pass@host/db`) from Neon's *Connection Details* — not the REST/Data API endpoint shown under Neon's *REST API* tab (`https://<endpoint>.apirest.<region>.aws.neon.tech/...`), which is a different product Prisma can't use.
