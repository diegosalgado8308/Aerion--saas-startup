...
export function runSimulation({ nodes, connections, steps, random = Math.random }) {
  ...
  // the raw material analyzeBalance() uses to spot bottlenecks.
  const flow = new Map(orderedConnections.map((c) => [c.id, { requested: 0, transferred: 0 }]));
 ...
 * pools/converters stuck full or empty (saturated/starved), each resource's
 * net flow in vs. out (via its Source and Drain nodes), and whether each
 * pool/converter is trending up, down, or flat over the run. Pure and
 * DB-free like runSimulation, and takes its result directly rather than
 * re-running the sim.
 */
