...
// Practical memory budget: each attachment is already capped at 10MB
// (MAX_ATTACHMENT_BYTES in lib/blob.js), but nothing capped how many a
// single task could accumulate — worst case, unbounded real Vercel Blob
// storage cost and an ever-growing list to render on the task detail page.
// 20 x 10MB = 200MB/task ceiling, generous for real use.
...
export async function addAttachment(taskId, workspaceId, uploaderId, { filename, url, size }) {
  ...
  if (attachmentCount >= MAX_ATTACHMENTS_PER_TASK) {
    throw new ValidationError(`A task can have at most ${MAX_ATTACHMENTS_PER_TASK} attachments.`);
  }
