...
describe("changePlan", () => {
  it("rejects a non-owner", async () => {
    const { workspace } = await makeTenant("changeplan-nonowner");
    createdWorkspaceIds.push(workspace.id);
    const { user: member } = await joinWorkspaceViaInvite({
      name: "Member", email: uniqueEmail("changeplan-nonowner"), password: "correct-horse-battery", inviteCode: workspace.inviteCode,
    });

    await expect(
      changePlan({ workspaceId: workspace.id, actingUserId: member.id, planKey: "BUSINESS" })
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("rejects when there's no active subscription to change", async () => {
    const { workspace, user: owner } = await makeTenant("changeplan-noactive");
    createdWorkspaceIds.push(workspace.id);

    await expect(
      changePlan({ workspaceId: workspace.id, actingUserId: owner.id, planKey: "BUSINESS" })
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("rejects switching to the plan already active", async () => {
    const { workspace, user: owner } = await makeTenant("changeplan-sameplan");
    createdWorkspaceIds.push(workspace.id);
    await setPlan(workspace.id, { plan: "PRO", subscriptionStatus: "active", stripeCustomerId: "cus_fake_sameplan", stripeSubscriptionId: "sub_fake_sameplan" });

    await expect(
      changePlan({ workspaceId: workspace.id, actingUserId: owner.id, planKey: "PRO" })
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("rejects an unknown plan key before ever reaching Stripe", async () => {
    const { workspace, user: owner } = await makeTenant("changeplan-badplan");
    createdWorkspaceIds.push(workspace.id);
    await setPlan(workspace.id, { plan: "PRO", subscriptionStatus: "active", stripeCustomerId: "cus_fake_badplan", stripeSubscriptionId: "sub_fake_badplan" });

    await expect(
      changePlan({ workspaceId: workspace.id, actingUserId: owner.id, planKey: "NOT_A_PLAN" })
    ).rejects.toBeInstanceOf(ValidationError);
  });
});
