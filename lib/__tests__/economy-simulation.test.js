...
describe("analyzeBalance bottlenecks", () => {
  it("flags a connection that couldn't get most of what it requested, classified as source-starved and moderate", () => {
    ...
    expect(balance.bottlenecks[0]).toMatchObject({
      connectionId: "b",
      requested: 5,
      transferred: 3,
      cause: "starved",
...
describe("analyzeBalance bottleneck root cause and severity", () => {
  it("classifies a destination-capped bottleneck as severe once the destination stays full most of the run", () => {
    // Same setup as the "stops accumulating once the pool hits capacity" runSimulation test.
    const nodes = [node("src", "SOURCE"), node("pool", "POOL", { initialValue: 0, capacity: 25 })];
    const connections = [conn("c1", "src", "pool", 10)];
    ...
    expect(balance.bottlenecks).toHaveLength(1);
    expect(balance.bottlenecks[0]).toMatchObject({ cause: "capped", severity: "severe", requested: 50, transferred: 25 });
  });

  it("classifies a source-starved bottleneck as severe once the source runs dry most of the run", () => {
    // Same setup as the "drains a finite pool" runSimulation test.
    const nodes = [node("pool", "POOL", { initialValue: 15 }), node("drain", "DRAIN")];
    const connections = [conn("c1", "pool", "drain", 10)];
    ...
    expect(balance.bottlenecks).toHaveLength(1);
    expect(balance.bottlenecks[0]).toMatchObject({ cause: "starved", severity: "severe", requested: 30, transferred: 15 });
  });

  it("classifies a moderate destination-capped bottleneck below the severe threshold", () => {
    const nodes = [node("src", "SOURCE"), node("pool", "POOL", { initialValue: 0, capacity: 20 })];
    const connections = [conn("c1", "src", "pool", 10)];

    // step1: 0->10 (room 20, no cap). step2: 10->20 (room 10, fills exactly, no cap).
    // step3: pool already at 20, room 0, so this step is capped.
    const balance = simulateAndAnalyze({ nodes, connections, steps: 3 });

    expect(balance.bottlenecks).toHaveLength(1);
    expect(balance.bottlenecks[0]).toMatchObject({ cause: "capped", severity: "moderate", requested: 30, transferred: 20 });
  });

  it("classifies a minor source-starved bottleneck below the moderate threshold", () => {
    const nodes = [node("pool", "POOL", { initialValue: 17 }), node("drain", "DRAIN")];
    const connections = [conn("c1", "pool", "drain", 10)];

    // step1: 17->7 (10 available, not starved). step2: only 7 left, so this step is starved.
    const balance = simulateAndAnalyze({ nodes, connections, steps: 2 });

    expect(balance.bottlenecks).toHaveLength(1);
    expect(balance.bottlenecks[0]).toMatchObject({ cause: "starved", severity: "minor", requested: 20, transferred: 17 });
  });
});
