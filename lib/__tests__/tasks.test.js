...
describe("markManyDueRemindersSent", () => {
  it("stamps dueReminderSentAt on every task in one batched call", async () => {
    const { workspace, project } = await makeTenantWithProject("mark-reminder-batch");
    createdWorkspaceIds.push(workspace.id);
    const taskA = await createTask(workspace.id, project.id, { title: "A", dueDate: new Date().toISOString() });
    const taskB = await createTask(workspace.id, project.id, { title: "B", dueDate: new Date().toISOString() });
    const taskC = await createTask(workspace.id, project.id, { title: "C", dueDate: new Date().toISOString() });

    await markManyDueRemindersSent([taskA.id, taskB.id]);

    const fetchedA = await getTaskForWorkspace(taskA.id, workspace.id);
    const fetchedB = await getTaskForWorkspace(taskB.id, workspace.id);
    const fetchedC = await getTaskForWorkspace(taskC.id, workspace.id);
    expect(fetchedA.dueReminderSentAt).not.toBeNull();
    expect(fetchedB.dueReminderSentAt).not.toBeNull();
    expect(fetchedC.dueReminderSentAt).toBeNull();
  });

  it("does nothing (no error, no query) for an empty list", async () => {
    await expect(markManyDueRemindersSent([])).resolves.toBeUndefined();
  });
});
...
describe("addAttachment", () => {
  ...
  it("rejects an attachment past the per-task cap (practical memory budget)", async () => {
    const { workspace, user, project } = await makeTenantWithProject("attachment-cap");
    createdWorkspaceIds.push(workspace.id);
    const task = await createTask(workspace.id, project.id, { title: "Task" });

    // Bulk-insert directly at the cap — fast setup, not itself under test.
    // The real addAttachment() call below is what's actually being verified.
    await prisma.attachment.createMany({
      data: Array.from({ length: 20 }, (_, i) => ({
        filename: `filler-${i}.pdf`, url: `https://example-blob.vercel-storage.com/filler-${i}.pdf`, size: 1, taskId: task.id,
      })),
    });

    await expect(
      addAttachment(task.id, workspace.id, user.id, { filename: "one-too-many.pdf", url: "https://x/one-too-many.pdf", size: 1 })
    ).rejects.toBeInstanceOf(ValidationError);
  });
});
